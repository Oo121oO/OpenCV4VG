/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class com_lulu_lulubox_opencv_OpenCVProcessor */

#ifndef _Included_com_lulu_lulubox_opencv_OpenCVProcessor
#define _Included_com_lulu_lulubox_opencv_OpenCVProcessor
#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     com_lulu_lulubox_opencv_OpenCVProcessor
 * Method:    detectHoughCircles
 * Signature: (Ljava/lang/Object;FIIII)[Ljava/lang/Object;
 */
JNIEXPORT jobject JNICALL Java_com_lulu_lulubox_opencv_OpenCVProcessor_detectHoughCircles
  (JNIEnv *, jobject, jobject, jfloat, jint, jint, jint, jint);

/*
 * Class:     com_lulu_lulubox_opencv_OpenCVProcessor
 * Method:    matchTemplate
 * Signature: (Ljava/lang/Object;Ljava/lang/Object;FIIII)[Ljava/lang/Object;
*/
JNIEXPORT jobject JNICALL Java_com_lulu_lulubox_opencv_OpenCVProcessor_matchTemplate
  (JNIEnv *, jobject, jobject, jobject, jfloat, jint, jint, jint, jint, jint);

#ifdef __cplusplus
}
#endif
#endif
